...
define(['jquery', 'underscore', 'jquery/ui'], function ($, _) {
    ...
    $.widget('custom.SwatchRenderer', {
        ...
        /**
         * @private: Rommy - 20170927: Hide Attributes
         */
        _rg_hideAttributes: function () {
            // 'FABRIC' attribute //
            this._rg_hideAttribute(138, 'fabric');
            
            // 'GENDER' attribute //
            this._rg_hideAttribute(139, 'gender');
            
            // this._rg_renderColorDropdown(90, 'color');
        },
        _rg_hideAttribute: function (id, selector) {
            var attr = $('.swatch-attribute.'+selector+'[attribute-id="'+id+'"]');
            if( attr.size() ) {
                attr.addClass('hide');
                attr.find('input[name="super_attribute['+id+']"]')
                    .val(attr.find('.swatch-select.'+selector+' option:eq(1)').attr('value'));
            }
        },
        
        /**
         * @private: Rommy - 20171005: Color Dropdown
         */
        _rg_renderColorDropdown: function (id, selector) {
            var attr = $('.swatch-attribute.'+selector+'[attribute-id="'+id+'"] select');
            if( attr.size() ) {
                attr.attr("data-showcolor", "true");
                attr.find("option").each(function(indx, obj) {
                    var that = $(this);
                    var val = that.attr("value");
                    
                    if(val != 0) {
                        that.attr("data-color", that.text());
                    }
                });
        ...
        _RenderControls: function () {
            ...
            $.each(this.options.jsonConfig.attributes, function () {
                ...
                if ($widget.options.enableControlLabel) {
                    label +=
                        '<span class="' + classes.attributeLabelClass + '">Select ' + item.label + '</span>' +
